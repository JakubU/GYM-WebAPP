...
@endpush
    ...
    <div class="container panel">
        ...
        <h1 class="rezervacie_H1">Prehľad rezervácií</h1>
        <table class="table table-hover">
            <thead>
            <tr>
                <th class="rezervacie_sekcie">Kurz</th>
                <th class="rezervacie_sekcie">Dátum, Čas</th>
                <th class="rezervacie_sekcie">Počet prihlásených</th>
                <th width="100"></th>
            </tr>
            </thead>
            <tbody>
            @foreach($reservations as $reservation)
                <tr>
                    <td class="rezervacia_nazov">{{ $reservation->name }}</td>
                    <td class="rezervacia_nazov2">{{ Carbon\Carbon::parse($reservation->near_day)->format('d.m.Y') }},
                        {{ Carbon\Carbon::parse($reservation->time_from)->format('H:i') }}
                        - {{ Carbon\Carbon::parse($reservation->time_to)->format('H:i') }}</td>
                    <td class="rezervacia_nazov2">{{ $reservation->near_tickets->count() }}</td>
                    <td>
                        <a href="{{ route('reservation', $reservation->id) }}" class="btn btn-primary rezervacia_btn">Pridaj sa!</a>
                    </td>
                </tr>
            @endforeach
            </tbody>
